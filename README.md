<img width="1207" height="620" alt="Screenshot (79)" src="https://github.com/user-attachments/assets/6495a4ca-5714-416f-9911-2e1b9164afb5" />
<img width="1253" height="659" alt="Screenshot (80)" src="https://github.com/user-attachments/assets/43930076-28fe-4860-9306-fb6217fda624" />
# Finance Analysis Project
Key Highlights:

Total Amount, Transactions, Average Transaction Value, Fees & Tax analysis
Monthly transaction trend analysis
Transaction status and transaction type analysis
Customer segment analysis
State-wise financial analysis
Gender-wise transaction analysis
Interactive filters for Year, Dynamic Metrics, Occupation & Category
Dedicated transaction-level detail page

🛠️ Tools & Skills Used:
Power BI | Power Query | DAX | Data Modeling | Data Visualization | Dashboard Design | measures
